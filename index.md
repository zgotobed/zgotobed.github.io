---
layout: default
title: Home
---

## About

I am a Physics PhD candidate at Colorado State University with a strong focus on
**numerical simulation, optical engineering, and the development of state of the art lasers**.

My work spans precision measurement, optical systems, and trapped ion quantum computing,
with hands-on experience building hardware, writing numerical simulations in Python and C++,
and applying these techniques to real-world problems. I am particularly interested in roles **working in quantum computing**, So that I can
apply my skills and experiences to help build the next generation of quantum machines.

---

## Technical Strengths

- **Scientific Computing:** Python, C++, Monte Carlo simulations, numerical optimization  
- **Data Analysis:** Maximum likelihood estimation, stochastic modeling, Fisher information techniques 
- **Experimental Systems:** Optical engineering, lasers, cryogenic systems, FPGAs  
- **Software Engineering:** Git, modular code design, testing, documentation  
- **CAD & Fabrication:** FreeCAD, OpenSCAD, CNC machining, 3D printing  

---

## Selected Projects

### Optical Clock Simulation Framework
Developed a Python and C++ simulation suite for evaluating novel optical clocking
methods. This framework reduced experimental iteration time by enabling rapid,
high-fidelity numerical testing before hardware implementation.

**Technologies:** Python, C++, Monte Carlo methods, Git

---

### PyOpticL — Code-to-CAD Optical Design
Contributed to an open-source library that converts Python code into fully
parametrized optical layouts suitable for CAD and 3D printing.

Used in real experimental systems with demonstrated extremely high (99.9%) gate fidelity.

**Technologies:** Python, FreeCAD, OpenSCAD, optical simulation  
**Preprint:** [arXiv:2501.14957](https://arxiv.org/abs/2501.14957)

---

### Trapped-Ion Experiment Control Framework
Refactored and extended a large ARTIQ-based codebase into a modular, class-driven
framework for running complex experimental sequences.

Improved usability, reproducibility, and long-term maintainability of the system.

**Technologies:** ARTIQ

---

## Industry Experience

**Energy Sciences, Inc.** — Dosimetry Intern  
Worked with industrial electron beam accelerators, assisting with product testing,
calibration, and customer-facing experimental runs.

Developed hands-on experience with high-voltage systems, experimental validation,
and cross-disciplinary collaboration.

---

## Publications

**Qubit operations using a modular optical system engineered with PyOpticL**  
*arXiv:2501.14957 (2025)*  
Myers et al., including Zack Gotobed  
[View on arXiv](https://arxiv.org/abs/2501.14957)

---

## Contact

If you’re interested in my work or would like to discuss potential opportunities,
feel free to reach out via email!
